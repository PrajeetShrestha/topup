...
float IMAGE_MIN_HEIGHT = 50;
float IMAGE_MIN_WIDTH = 50;
...
- (void)initViews {
    ...
    //the image
    imageView = [[UIImageView alloc] initWithFrame:CGRectOffset(subviewFrame, 0, 100)];
    ...
    CGFloat height = 30;
    CGPoint topPoint = CGPointMake(offset, centerInView.y - height/2);
    CGPoint bottomPoint = CGPointMake(offset, centerInView.y + height/2);
    CGPoint topRight = CGPointMake(screenWidth - offset, centerInView.y - height/2);
    CGPoint bottomRight = CGPointMake(screenWidth - offset, centerInView.y + height/2);

    topLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(topPoint.x,topPoint.y,controlPointSize)];
    bottomLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(bottomPoint.x,bottomPoint.y,controlPointSize)];
    bottomRightPoint = [self createControlPointAt:SquareCGRectAtCenter(bottomRight.x,bottomRight.y ,controlPointSize) ];
    topRightPoint = [self createControlPointAt:SquareCGRectAtCenter(topRight.x,topRight.y,controlPointSize)];
